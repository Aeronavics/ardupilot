...
#define HASTEN_SPEED_DEFAULT                1250.0f // default hasten speed in cm/s
#define HASTEN_SPEED_MIN                    20.0f   // minimum hasten speed in cm/s
#define HASTEN_ACCEL_MAX_DEFAULT            500.0f  // default acceleration in hasten mode
#define HASTEN_BRAKE_ACCEL_DEFAULT          250.0f  // minimum acceleration in hasten mode
#define HASTEN_BRAKE_JERK_DEFAULT           500.0f  // maximum jerk in cm/s/s/s in hasten mode
#define HASTEN_BRAKE_START_DELAY_DEFAULT    1.0f    // delay (in seconds) before hasten braking begins after sticks are released
#define HASTEN_VEL_CORRECTION_MAX           200.0f  // max speed used to correct position errors in hasten
#define HASTEN_POS_CORRECTION_MAX           200.0f  // max position error in hasten
#define HASTEN_ACTIVE_TIMEOUT_MS            200     // hasten controller is considered active if it has been called within the past 200ms (0.2 seconds)

const AP_Param::GroupInfo AC_Hasten::var_info[] = {

    // @Param: ANG_MAX
    // @DisplayName: Hasten Angle Max
    // @Description: Hasten maximum lean angle. Set to zero for 2/3 of PSC_ANGLE_MAX or ANGLE_MAX
    // @Units: deg
    // @Range: 0 45
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("ANG_MAX",  1, AC_Hasten, _angle_max, 0.0f),

    // @Param: SPEED
    // @DisplayName: Hasten Horizontal Maximum Speed
    // @Description: Defines the maximum speed in cm/s which the aircraft will travel horizontally while in hasten mode
    // @Units: cm/s
    // @Range: 20 3500
    // @Increment: 50
    // @User: Standard
    AP_GROUPINFO("SPEED", 2, AC_Hasten, _speed_cms, HASTEN_SPEED_DEFAULT),

    // @Param: ACC_MAX
    // @DisplayName: Hasten maximum correction acceleration
    // @Description: Hasten maximum correction acceleration in cm/s/s.  Higher values cause the copter to correct position errors more aggressively.
    // @Units: cm/s/s
    // @Range: 100 981
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("ACC_MAX", 3, AC_Hasten, _accel_cmss, HASTEN_ACCEL_MAX_DEFAULT),

    // @Param: BRK_ACCEL
    // @DisplayName: Hasten braking acceleration
    // @Description: Hasten braking acceleration in cm/s/s. Higher values stop the copter more quickly when the stick is centered.
    // @Units: cm/s/s
    // @Range: 25 250
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("BRK_ACCEL", 4, AC_Hasten, _brake_accel_cmss, HASTEN_BRAKE_ACCEL_DEFAULT),

    // @Param: BRK_JERK
    // @DisplayName: Hasten braking jerk
    // @Description: Hasten braking jerk in cm/s/s/s. Higher values will remove braking faster if the pilot moves the sticks during a braking maneuver.
    // @Units: cm/s/s/s
    // @Range: 500 5000
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("BRK_JERK", 5, AC_Hasten, _brake_jerk_max_cmsss, HASTEN_BRAKE_JERK_DEFAULT),

    // @Param: BRK_DELAY
    // @DisplayName: Hasten brake start delay (in seconds)
    // @Description: Hasten brake start delay (in seconds)
    // @Units: s
    // @Range: 0 2
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("BRK_DELAY",  6, AC_Hasten, _brake_delay, HASTEN_BRAKE_START_DELAY_DEFAULT),
...
// their values.
//
AC_Hasten::AC_Hasten(const AP_InertialNav& inav, const AP_AHRS_View& ahrs, AC_PosControl& pos_control, const AC_AttitudeControl& attitude_control) :
    _inav(inav),
    _ahrs(ahrs),
...
void AC_Hasten::init_target(const Vector2f& position)
{
    sanity_check_params();

    // initialise position controller speed and acceleration
    _pos_control.set_correction_speed_accel_xy(HASTEN_VEL_CORRECTION_MAX, _accel_cmss);
    _pos_control.set_pos_error_max_xy_cm(HASTEN_POS_CORRECTION_MAX);
...
void AC_Hasten::soften_for_landing()
{
...
void AC_Hasten::set_pilot_desired_acceleration(float euler_roll_angle_cd, float euler_pitch_angle_cd)
{
...
void AC_Hasten::get_stopping_point_xy(Vector2f& stopping_point) const
{
    Vector2p stop;
...
/// get maximum lean angle when using Hasten
float AC_Hasten::get_angle_max_cd() const
{
...
/// run the Hasten controller
void AC_Hasten::update(bool avoidance_on)
{
...
void AC_Hasten::sanity_check_params()
{
    _speed_cms = MAX(_speed_cms, HASTEN_SPEED_MIN);
...
void AC_Hasten::calc_desired_velocity(float nav_dt, bool avoidance_on)
{
    float ekfGndSpdLimit, ekfNavVelGainScaler;
    AP::ahrs_navekf().getEkfControlLimits(ekfGndSpdLimit, ekfNavVelGainScaler);

    // calculate a Hasten speed limit which is the minimum of the value set by the HASTEN_SPEED
    // parameter and the value set by the EKF to observe optical flow limits
    float gnd_speed_limit_cms = MIN(_speed_cms, ekfGndSpdLimit*100.0f);
    gnd_speed_limit_cms = MAX(gnd_speed_limit_cms, HASTEN_SPEED_MIN);
    ...
    // get Hastens desired velocity from the position controller where it is being stored.
    ...
    if (!is_zero(desired_speed)) {
        ...
        if (_desired_accel.is_zero()) {
            if ((AP_HAL::millis()-_brake_timer) > _brake_delay * 1000.0f) {
                ...
                hasten_brake_accel = constrain_float(sqrt_controller(desired_speed, brake_gain, _brake_jerk_max_cmsss, nav_dt), 0.0f, _brake_accel_cmss);
            }
        } else {
            hasten_brake_accel = 0.0f;
            _brake_timer = AP_HAL::millis();
        }
        _brake_accel += constrain_float(hasten_brake_accel-_brake_accel, -_brake_jerk_max_cmsss*nav_dt, _brake_jerk_max_cmsss*nav_dt);
        hasten_accel_brake = desired_vel_norm*_brake_accel;
    ...
    _desired_accel -= hasten_accel_brake;
