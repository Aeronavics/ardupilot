...
public:
    ...
    /// Constructor
    AC_Hasten(const AP_InertialNav& inav, const AP_AHRS_View& ahrs, AC_PosControl& pos_control, const AC_AttitudeControl& attitude_control);
    ...
    /// get horizontal distance to hasten target in cm
...
protected:
    ...
    AP_Float    _speed_cms;             // maximum horizontal speed in cm/s while in hasten
    AP_Float    _accel_cmss;            // hasten's max acceleration in cm/s/s
    AP_Float    _brake_accel_cmss;      // hasten's acceleration during braking in cm/s/s
    AP_Float    _brake_jerk_max_cmsss;
    AP_Float    _brake_delay;           // delay (in seconds) before hasten braking begins after sticks are released

    // hasten controller internal variables
